...
/**
 * Class containing various static utility methods and generators dealing with numerical values.
 *
 * @author Lee Mracek
 */
public final class Values {
  ...
  /**
   * Compare two numbers to within a certain number of bits.
   *
   * @param a    the first number
   * @param b    the second number
   * @param bits the number of bits of accuracy desired
   * @return 0 if the numbers are the same, 1 if a is greater, -1 if b is greater
   */
  ...
  private static int fuzzyCompare(double a, double b, double tolerance) {
    if (tolerance < 0.0) {
      throw new IllegalArgumentException("The tolerance may not be negative");
    }
    double difference = a - b;
    return difference <= tolerance ? 0 : (int) Math.signum(difference);
  }
  ...
  /**
   * Construct and return an {@link DoubleFunction} which limits a value to between two
   * other values.
   *
   * @param minimum the minimum possible value
   * @param maximum the maximum possible value; must be greater than the minimum
